...
import type { CreateRatingBody, RatingView } from "../models/ratings.model.js";
...
function toView(row: RatingRow): RatingView {
  return {
    id: row.id,
    stars: row.stars,
...
export const ratingsService = {
  create(auth: AuthContext, assignmentId: string, data: CreateRatingBody): RatingView {
    const assignment = db.prepare(`
      SELECT a.id, a.student_id, a.status, r.family_id
      FROM assignments a JOIN activity_requests r ON a.request_id = r.id
      WHERE a.id = ?
    `).get(assignmentId) as {
      id: string; student_id: string; status: string; family_id: string;
    } | undefined;
    ...
    if (existing) throw new AppError("Ya calificaste esta visita", 409, "ALREADY_RATED");
  ...
  listForStudent(studentId: string): RatingView[] {
    const rows = db.prepare(`
      SELECT r.*, u.name AS author_name FROM ratings r
      LEFT JOIN users u ON r.author_user_id = u.id
      WHERE r.student_id = ? ORDER BY r.created_at DESC
    `).all(studentId) as RatingRow[];
