...
function award(studentId: string, slug: string): void {
  const badge = db.prepare("SELECT id FROM badges WHERE slug = ?").get(slug) as { id: string } | undefined;
  if (!badge) return;
...
export const badgesService = {
  listForStudent(studentId: string): BadgeView[] {
    ...
    `).all(studentId) as Array<{ slug: string; title: string; description: string; icon: string; earned_at: string }>;
    return rows.map((r) => ({
  ...
  evaluate(studentId: string): void {
    ...
    `).get(studentId, studentId, studentId, studentId, studentId, studentId) as {
      completed: number; cancelled: number; total_hours: number;
      rating_count: number; avg_stars: number | null; urgent_completed: number;
    };

    if (stats.completed >= 1) award(studentId, "primera_visita");
    if (stats.total_hours >= 10) award(studentId, "10_horas");
    if (stats.rating_count >= 3 && (stats.avg_stars ?? 0) >= 4.8) award(studentId, "5_estrellas");
    if (stats.completed >= 5 && stats.cancelled === 0) award(studentId, "confiable");
    if (stats.urgent_completed >= 3) award(studentId, "urgencias");
  },
};
