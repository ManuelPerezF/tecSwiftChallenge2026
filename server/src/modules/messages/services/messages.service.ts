...
import type { SendMessageBody, ReplyMessageBody } from "../models/messages.model.js";
...
function normalize(row: MessageRow) {
  return {
    id: row.id,
...
export const messagesService = {
  // Family → Student
  send(auth: AuthContext, data: SendMessageBody) {
    if (!auth.familyId) throw new UnauthorizedError("Solo las familias pueden enviar mensajes");
  ...
  reply(auth: AuthContext, data: ReplyMessageBody) {
    if (!auth.studentId) throw new UnauthorizedError("Solo los becarios pueden responder");

    const targetUser = db.prepare("SELECT id FROM users WHERE id = ?").get(data.toUserId);
    if (!targetUser) throw new NotFoundError("Destinatario no encontrado");
    ...
    if (!existing) throw new UnauthorizedError("Solo puedes responder a familias que te han escrito");
  ...
  inbox(auth: AuthContext) {
    if (!auth.studentId) throw new UnauthorizedError("Solo los becarios tienen bandeja de entrada");
    ...
    const rows = db.prepare(`
      ...
      LIMIT 100
    `).all(auth.studentId) as MessageRow[];
  ...
  conversations(auth: AuthContext) {
    if (!auth.familyId) throw new UnauthorizedError("Solo las familias pueden ver conversaciones");
    ...
    `).all(auth.familyId, auth.familyId) as {
      studentId: string; studentName: string;
      lastBody: string; lastAt: string; unreadCount: number;
    }[];
  ...
  thread(auth: AuthContext, otherId: string) {
    let rows: MessageRow[];
    ...
    if (auth.familyId) {
      ...
      `).all(otherId, auth.familyId, auth.familyId) as MessageRow[];
      ...
      rows = db.prepare(`
        ...
        ORDER BY m.created_at ASC
      `).all(auth.studentId, otherId, otherId) as MessageRow[];
  ...
  unreadCount(auth: AuthContext): number {
    if (!auth.studentId) return 0;
    const result = db.prepare(`
      SELECT COUNT(*) AS n FROM messages
      WHERE to_student_id = ? AND to_user_id IS NULL AND read_at IS NULL
    `).get(auth.studentId) as { n: number };
    return result.n;
  },
};
