...
interface AssignmentRow {
  ...
  student_id: string;
  status: "approved" | "en_camino" | "iniciada" | "esperando_confirmacion_fin" | "completada" | "cancelada";
  approved_at: string;
...
function assertNotTooEarly(scheduledDate: string): void {
...
function notifyFamily(familyId: string, type: string, title: string, body: string, data: Record<string, unknown> = {}): void {
  const members = db
    .prepare("SELECT user_id FROM family_members WHERE family_id = ?")
    .all(familyId) as Array<{ user_id: string }>;
...
export const assignmentsService = {
  ...
  completar(auth: AuthContext, id: string): AssignmentView {
    ...
    notifyFamily(
      ...
      "completion_pending",
      "¿El servicio terminó?",
      `${row.student_name} marcó como terminada la visita de ${row.activity_type} con ${row.elderly_name ?? "tu familiar"}. Confirma para registrar las horas.`,
      { assignmentId: id },
    );
    broadcastAssignmentStatus(id);
    return toView(getRow(id));
   ...
   */
  confirmCompletion(auth: AuthContext, id: string): AssignmentView {
    const row = getRow(id);
    ...
    if (!isFamily && !isElderly) throw new UnauthorizedError("No es una visita de tu familia");
    ...
    const studentUser = db
      ...
      .get(row.student_id) as { user_id: string } | undefined;
    if (studentUser) {
      notificationsService.create(
        studentUser.user_id,
        "completion_confirmed",
        "¡Visita confirmada!",
        "La familia confirmó el fin del servicio. Tus horas reales ya fueron registradas.",
        { assignmentId: id },
      );
    }
  ...
  cancelByStudent(auth: AuthContext, id: string): AssignmentView {
    const row = getRow(id);
    if (row.student_id !== auth.studentId) throw new UnauthorizedError("No es tu visita");
    ...
    notifyFamily(
      ...
      "El becario canceló la visita",
      `${row.student_name} canceló la visita de ${row.activity_type}. La solicitud volvió a abrirse y la lista de espera está disponible.`,
      { requestId: row.request_id, assignmentId: id },
    );
    return view;
  },

  performCancellation(row: AssignmentRow, id: string): AssignmentView {
    let reopened = false;
    const tx = db.transaction(() => {
  ...
  proposeChange(auth: AuthContext, id: string, proposedDate: string): { proposalId: string } {
    const row = getRow(id);
    if (row.student_id !== auth.studentId) throw new UnauthorizedError("No es tu visita");
    if (resolveStatus(row) !== "approved") {
    ...
    const existing = db
      ...
      .get(id);
    if (existing) throw new AppError("Ya hay una propuesta pendiente", 409, "PROPOSAL_PENDING");
    ...
    notifyFamily(
      ...
      `${row.student_name} propone cambiar la visita de ${row.activity_type} a otra hora. Revísala para aceptar o rechazar.`,
      { assignmentId: id, proposalId, proposedDate },
    );
    return { proposalId };
  },

  /** Propuesta pendiente de un assignment (familia o becario). */
  getPendingProposal(auth: AuthContext, id: string): { id: string; proposedDate: string; createdAt: string } | null {
    const row = getRow(id);
    ...
    const proposal = db
      ...
      .get(id) as { id: string; proposed_date: string; created_at: string } | undefined;
  ...
  respondToProposal(auth: AuthContext, proposalId: string, accept: boolean): { ok: true } {
    const proposal = db.prepare(`
      ...
      WHERE p.id = ?
    `).get(proposalId) as {
      id: string; assignment_id: string; proposed_date: string; status: string;
      proposed_by: string; request_id: string; family_id: string; student_name: string;
    } | undefined;

    if (!proposal) throw new NotFoundError("Propuesta no encontrada");
    if (proposal.family_id !== auth.familyId) throw new UnauthorizedError("No es una visita de tu familia");
    if (proposal.status !== "pending") throw new AppError("La propuesta ya fue resuelta", 409, "ALREADY_RESOLVED");
    ...
    notificationsService.create(
      ...
      accept ? "change_accepted" : "change_rejected",
      accept ? "Cambio de horario aceptado" : "Cambio de horario rechazado",
      accept
        ? "La familia aceptó tu propuesta. La visita quedó reagendada."
        : "La familia prefirió mantener el horario original.",
