...
import { createEventTypeBodySchema, type EventTypeView } from "../models/eventTypes.model.js";
...
function toView(row: EventTypeRow): EventTypeView {
...
eventTypesRouter.post("/", requireRole("organizer"), (req, res, next) => {
  try {
    ...
    if (!parsed.success) throw new ValidationError("Tipo de evento inválido (label 2-40 caracteres)");

    const base = slugify(parsed.data.label);
    if (!base) throw new ValidationError("Nombre de tipo inválido");
