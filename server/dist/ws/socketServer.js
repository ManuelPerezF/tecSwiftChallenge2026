...
import { resolveAuthContext } from "../shared/middlewares/auth.middleware.js";
...
function assignmentAccess(assignmentId) {
    return db.prepare(`
...
function canWatch(auth, row) {
    return (auth.familyId === row.family_id ||
...
export function broadcastLocation(assignmentId) {
...
export function broadcastAssignmentStatus(assignmentId) {
  ...
  `).get(assignmentId);
    if (!row)
        return;
    const status = row.status === "en_camino" && row.inicio_solicitado_at && !row.checkin_at
...
function handleLocationUpdate(ws, state, msg) {
    ...
    const now = Date.now();
    if (now - (lastLocationAt.get(throttleKey) ?? 0) < LOCATION_THROTTLE_MS)
        return;
        ...
        ws.on("message", (raw) => {
            ...
            switch (msg.type) {
                ...
                case "assignment:unsubscribe":
                    if (msg.assignmentId)
                        state.subscribedAssignments.delete(msg.assignmentId);
                    break;
                default:
