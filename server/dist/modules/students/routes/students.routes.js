...
studentsRouter.put("/me/tags", requireRole("student"), (req, res, next) => {
    try {
        if (!req.auth?.studentId)
            throw new UnauthorizedError("Solo estudiantes");
        const parsed = tagsBodySchema.safeParse(req.body);
        if (!parsed.success)
            throw new ValidationError("Tags inválidos (máx. 12, 30 caracteres c/u)");
...
studentsRouter.put("/me/availability", requireRole("student"), (req, res, next) => {
    try {
        ...
        if (!parsed.success)
            throw new ValidationError("Disponibilidad inválida");
...
function fetchStudentById(id, res, next) {
    try {
...
studentsRouter.get("/me", (req, res, next) => {
    ...
    if (!studentId) {
        res.status(403).json({ error: "No eres un becario" });
        return;
    }
    fetchStudentById(studentId, res, next);
});
studentsRouter.get("/:id", (req, res, next) => {
    fetchStudentById(req.params.id, res, next);
});
